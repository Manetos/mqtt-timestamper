...
describe('Timestamper', function() {
    ...
    it('should not forward retained messages', function() {
        /* jshint nonew: false */
        new Timestamper(this.client, '#', sinon.stub().returns('newTopic'));
        this.client.on.withArgs('message').yield('woop', JSON.stringify({}), {
            retain: true,
    ...
    describe('publish options', function() {
        it('shoudl pass through publishOptions to publish', function() {
            ...
            assert.deepEqual(this.client.publish.getCall(0).args[2], publishOptions);
        });

        it('should forward qos level', function() {
            /* jshint nonew: false */
            new Timestamper(this.client, '#', sinon.stub().returns('newTopic'));
            this.client.on.withArgs('message').yield('woop', JSON.stringify({}), {
                retain: false,
            ...
            assert.strictEqual(this.client.publish.getCall(0).args[2].qos, 2);
        });
    });
